...
export async function interactive(deps: CliDeps, options: { confirm?: string | undefined; assumeYes?: boolean } = {}): Promise<number> {
  ...
  const keyword = await autocomplete({
    ...
    filter: (search, option) => {
      ...
      try {
        const found = apps.find((app) => app.path === String(option.value));
        return found ? scoreMatch(found, search) > 0 : true;
      } catch {
        return true;
      }
    },
  ...
  const selectedPath = String(keyword);
